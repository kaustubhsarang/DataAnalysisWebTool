	...
	public String fetchTables() {
		try {
			...
			if (rs != null) {
				...
				renderTableList();
				statsBean.setTableList(tableList);
				statsBean.setRenderTablename(true);
				export.setCmd("tableList");
				export.addCmd();
				export.setCmd("\n");
				export.addCmd();
				export.setCmd("use");
		...
		try {
			...
			if (tableSelected.isEmpty()) {
				...
				if(worldTables.contains(tableSelected))
					...
					sqlQuery = "select * from "+dbBean.getDatabase()+"." + tableSelected;
				resultSet = dbAccess.selectQueryProcessing(sqlQuery);
				userQuery = sqlQuery;
				if (resultSet != null) {
					buildMetaData();
					renderTabledata = true;
					export.setCmd(tableSelected+"\n");
					export.addCmd();
					return "SUCCESS";
				} else {
					message = dbAccess.getMessage();
		...
		try {
			...
			if (tableSelected.isEmpty()) {
				...
				if (resultSet != null) {
					...
					statsBean.setColumnsList(columnsList);
					statsBean.setColumnRender(true);
					columnRender = true;
					disableColumnData = false;
					export.setCmd("columnList\n");
					export.addCmd();
		...
		try {
			if (tableSelected != null && columnSelected != null) {
				...
				for (int i = 0; i < columnSelected.size(); i++) {
					...
					export.setCmd(columnSeperated.get(i).toString()+"\t");
					export.addCmd();
				}
				export.setCmd("\n");
				export.addCmd();
				...
				else sqlQuery = "select " + sqlQuery + " from "+dbBean.getDatabase()+"." + tableSelected;
...
//				}
	...
	/*public String dropTable() {
		try {
			...
			if (!worldTables.contains(tableSelected)) {
				...
				String sqlQuery = "drop table world." + tableSelected;
	...
	public void setTableSelected(String tableSelected) {
		statsBean.setTableSelected(tableSelected);
	...
	public void setColumnSelected(List<String> columnSelected) {
		statsBean.setColumnSelected(columnSelected);
